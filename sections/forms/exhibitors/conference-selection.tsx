...
export default function ConferenceSelection({
  ...
  setSubmissionAction: React.Dispatch<React.SetStateAction<ExhibitorSubmissionType>>;
  error?: string;
}) {
    ...
    <div id="conference-selection" className="w-full form_section_wrapper">
      <div className="form-section-spacing">
        ...
        <p className="text-xs sm:text-sm text-white">
          Choose the conference where you wish to exhibit
        </p>
        {error && (
          <p className="text-red-400 text-sm font-medium mt-2 flex-center-gap-1">
          ...
          return (
            ...
            >
              <div className="flex gap-3 sm:gap-4 p-3 sm:p-4">
                {/* Poster Image - 2:3 aspect ratio (industry standard for posters) */}
                <div className="relative w-20 h-30 sm:w-24 sm:h-36 flex-shrink-0">
                  ...
                  {isSelected && (
                    <div className="absolute inset-0 bg-yellow-400/20 border-2 border-yellow-400 rounded-md flex items-center justify-center">
                      <div className="bg-yellow-400 text-stone-900 rounded-full w-8 h-8 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className={clsx(
                    ...
                    isSelected ? "text-yellow-400" : "text-yellow-400"
                  )}>
                    {conf.title}
                  </h3>
